...
namespace Alsing.SourceCode
{
    ...
    public class SyntaxDefinitionLoader
    {
        ...
        private void ParseStyle(XmlNode node)
        {
            ...
            if (BackColor != "")
            {
                if (System.Text.RegularExpressions.Regex.IsMatch(BackColor, "#[0-9a-fA-F]{6}"))
                    st.BackColor = Color.FromArgb(Int32.Parse(BackColor.Substring(1), System.Globalization.NumberStyles.HexNumber));
                else
                    st.BackColor = Color.FromName(BackColor);
            }

            if (System.Text.RegularExpressions.Regex.IsMatch(ForeColor, "#[0-9a-fA-F]{6}"))
                st.ForeColor = Color.FromArgb(Int32.Parse(ForeColor.Substring(1), System.Globalization.NumberStyles.HexNumber));
            else
